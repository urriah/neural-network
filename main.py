...
class Optimizer_SGD:
	...
	def update_params(self, layer):
		if self.momentum:
			...
			weight_updates = \
				self.momentum * layer.weights_momentums - \
...
class Optimizer_Adagrad:
	...
	def update_params(self, Layer):
		if not hasattr(layer, 'weight_cache'):
			...
			layer.bias_cach = np.zeros_like(layer.biases)
		...
		layer.biases += -self.current_learning_rate * \
				layer.dbiases / \
				(np.sqrt(layer.bias_cach) + self.epsilon
...
class Optimizer_Adam:
	...
	def pre_update_params(self):
		if self.decay:
			self.current_learning_rate = self.learning_rate * \
				(1. / (1. + self.decat * self.iterations))
